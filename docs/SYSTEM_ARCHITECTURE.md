...
`createOperationalRefreshController()` owns a single 10-second interval per
mounted live surface. It pauses on `document.hidden`, refreshes on visibility and
focus return, coalesces events, suppresses overlapping work, ignores transient
background failure, and removes all listeners/timers on unmount.
