...
import { startOrderPreparation } from "@/lib/kitchen/service";
...
export async function POST(_request, { params }) {
  ...
  if (role !== "ADMIN" && role !== "CHEF") {
    return NextResponse.json(
      { ok: false, message: "Kitchen access is required." },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    console.error("[kitchen-start-preparing]", {
      category: error?.code || error?.name || "mutation_failed",
    });
    return NextResponse.json(
      { ok: false, message: "Preparation could not be started." },
      { status: 500 }
    );
  }
}
