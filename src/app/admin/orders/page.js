...
import { ACTIVE_ORDER_STATUSES, HISTORY_ORDER_STATUSES, listAdminOrders } from "@/lib/admin/orders";
...
const VIEWS = {
  active: { label: "Active Orders", statuses: ACTIVE_ORDER_STATUSES },
...
function getView(value) {
  ...
  return Object.hasOwn(VIEWS, candidate) ? candidate : "active";
}
...
export default async function AdminOrdersPage({ searchParams }) {
  ...
  return <>
    ...
    {view === "analytics" ? <AdminOrderAnalytics metrics={metrics} filters={analytics.values} errors={analytics.errors} /> : <>
      <ContentSection title={`${VIEWS[view].label} Filters`} description={view === "active" ? "Filter orders still requiring operational attention." : "Filter completed and cancelled order records."} className="admin-section">
      ...
      <ContentSection title={view === "active" ? "Active Order Queue" : "Order History"} description={result ? `${result.total} matching order${result.total === 1 ? "" : "s"}.` : "Order data is currently unavailable."} className="admin-section">
        {result ? <><AdminOrderTable orders={result.rows} emptyMessage={view === "active" ? "There are no active orders." : "There is no completed or cancelled order history yet."} /><AdminPagination basePath="/admin/orders" page={result.page} pageSize={result.pageSize} total={result.total} query={{ ...orderFilters.values, view }} /></> : <p className="admin-data-error">Orders could not be loaded. No data has been changed.</p>}
      </ContentSection>
    </>}
  </>;
}
